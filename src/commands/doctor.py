...
def full_optimization(project_path: Path, dry_run: bool = False) -> FixResult:
    ...
    from ..optimizer.heavy_mover import move_heavy_files, generate_config_paths
    ...
    if not moveable:
        ...
        try:
            ...
            result.files_moved = len(move_result.files_moved)
            result.tokens_saved = sum(f.estimated_tokens for f in move_result.files_moved)
    ...
    if not dry_run and moveable:
        try:
            patch_report = patch_project(project_path, moveable)
            result.files_patched = len(patch_report.files_patched)
            print(f"  Patched {result.files_patched} files")
            
            if patch_report.dynamic_path_warnings:
                # Check if symlinks were created
                symlinks_ok = 'move_result' in dir() and move_result.symlinks_created
    ...
    if not dry_run and moveable:
        try:
            generate_fox_trace_map(project_path, moveable)
